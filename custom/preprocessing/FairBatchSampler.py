"""
This code reference to: https://github.com/yuji-roh/fairbatch
"""
import sys, os
import numpy as np
import math
...
class FairBatch(Sampler):
    ...
    def __init__(
        ...
        self.x_data = x_tensor.float().to(DEVICE)
        self.y_data = y_tensor.float().to(DEVICE)
        self.z_data = z_tensor.float().to(DEVICE)
        ...
        for tmp_yz in self.yz_tuple:
            ...
            if self.yz_index[tmp_yz].dim() == 0:
                self.yz_index[tmp_yz] = self.yz_index[tmp_yz].unsqueeze(0)
        # Length information
        self.z_len = {}
        self.y_len = {}
        self.yz_len = {}
        ...
        self.lb1 = (self.S[1, 1]) / (self.S[1, 1] + (self.S[1, 2]))
        self.lb2 = (self.S[0, 1]) / (self.S[0, 1] + (self.S[0, 2]))

    def et_model(self, new_model):
    ...
    def adjust_lambda(self):
        ...
        self.model.eval()
        logit = self.model(self.x_data.float().to(DEVICE)).flatten()
        ...
        if self.fairness_type == "eqopp":
            ...
            for tmp_yz in self.yz_tuple:
                yhat_yz[tmp_yz] = float(torch.sum(eo_loss[self.yz_index[tmp_yz]])) / (
                    self.yz_len[tmp_yz] + 0.1
                )

            for tmp_y in self.y_item:
                yhat_y[tmp_y] = float(torch.sum(eo_loss[self.y_index[tmp_y]])) / (
                    self.y_len[tmp_y] + 0.1
                )

            # lb1 * loss_z1 + (1-lb1) * loss_z0

            if yhat_yz[(1, 1)] > yhat_yz[(1, 2)]:
                self.lb1 += self.alpha
            else:
                self.lb1 -= self.alpha
            ...
            for tmp_yz in self.yz_tuple:
                yhat_yz[tmp_yz] = (
                    float(torch.sum(eo_loss[self.yz_index[tmp_yz]]))
                    / self.yz_len[tmp_yz]
                )

            for tmp_y in self.y_item:
                yhat_y[tmp_y] = (
                    float(torch.sum(eo_loss[self.y_index[tmp_y]])) / self.y_len[tmp_y]
                )

            y1_diff = abs(yhat_yz[(1, 1)] - yhat_yz[(1, 2)])
            y0_diff = abs(yhat_yz[(-1, 1)] - yhat_yz[(-1, 2)])
            ...
            ones_tensor = torch.FloatTensor(ones_array).float().to(DEVICE)
            dp_loss = criterion(
                (F.tanh(logit) + 1) / (2 + 1e-6), ones_tensor
            )  # Note that ones tensor puts as the true label

            for tmp_yz in self.yz_tuple:
                yhat_yz[tmp_yz] = (
                    float(torch.sum(dp_loss[self.yz_index[tmp_yz]]))
                    / self.z_len[tmp_yz[1]]
                )

            y1_diff = abs(yhat_yz[(1, 1)] - yhat_yz[(1, 2)])
            y0_diff = abs(yhat_yz[(0, 1)] - yhat_yz[(0, 2)])
    ...
    def select_batch_replacement(
        self, batch_size, full_index, batch_num, replacement=False
    ):
                ...
                if start_idx + batch_size > len(full_index):
                    select_index.append(
                        np.concatenate(
                            (
                                tmp_index[start_idx:],
                                tmp_index[: batch_size - (len(full_index) - start_idx)],
                            )
                        )
                    )
    ...
    def __iter__(self):
        ...
        if self.fairness_type == "original":
            ...
            if self.fairness_type == "eqopp":
                ...
                each_size[(1, 1)] = round(self.lb1 * (self.S[(1, 1)] + self.S[(1, 2)]))
                each_size[(1, 2)] = round(
                    (1 - self.lb1) * (self.S[(1, 1)] + self.S[(1, 2)])
                )
                each_size[(0, 1)] = round(self.S[(0, 1)])
                each_size[(0, 2)] = round(self.S[(0, 2)])
                ...
                each_size[(0, 1)] = round(self.lb2 * (self.S[(0, 1)] + self.S[(0, 2)]))
                each_size[(0, 2)] = round(
                    (1 - self.lb2) * (self.S[(0, 1)] + self.S[(0, 2)])
                )
